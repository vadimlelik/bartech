...
**Важно:** Если контейнеры уже запущены, их можно оставить работающими, так как используется DNS-валидация (не требуется доступ к веб-серверу). Однако, если вы получаете сертификаты впервые, рекомендуется остановить контейнеры для избежания потенциальных конфликтов с volumes:

```bash
# Если контейнеры уже запущены, остановите их (опционально)
