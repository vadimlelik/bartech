...
4. **Убедитесь, что режим установлен на "Full" или "Full (strict)"**
   ...
   ❌ **"Flexible"** - НЕ ПОДХОДИТ! Cloudflare подключается к вашему серверу по HTTP, а не HTTPS
   ✅ **"Full"** - Cloudflare подключается по HTTPS, но не проверяет сертификат
   ✅ **"Full (strict)"** - Рекомендуется! Cloudflare подключается по HTTPS и проверяет валидность сертификата
...
### Шаг 3: Проверьте wildcard сертификат на сервере
...
### Шаг 5: Проверьте доступность сервера напрямую
...
### Вариант 1: Временно отключите Cloudflare Proxy
...
2. **Cloudflare SSL/TLS Analyzer**:
   - В Cloudflare Dashboard → **SSL/TLS** → **Edge Certificates**
